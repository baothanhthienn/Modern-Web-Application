...
The frontend now waits for successful room join before enabling send, leaves prior rooms when changing conversations or navigating away, joins again on each socket reconnect, shows socket connection errors, and appends acknowledged sender messages while deduplicating the broadcast copy by message ID.
...
| client -> server | `community:leave` | `{ community: string }` | `{ success: true, community: string }` |
...
| client -> server | `direct:leave` | `{ username: string }` | `{ success: true, with: string }` |
...
The backend implementation was updated to require joined rooms for sends, remove subscriptions with leave events, and recheck community membership at delivery time. The backend agent should retain the following verification before deployment:
...
4. Test that `community:leave` and `direct:leave` remove recipients from room delivery and reject subsequent sends from a sender who has left.
